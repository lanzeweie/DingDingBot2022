...
class xiaomiyundong():
    ...
    def login(user,password):
        ...
        try:
            ...
            code = xiaomiyundong.get_code(location)
        except:
            return 0,0
    ...
    def main(user, passwd, step):
        ...
        login_token = 0
        login_token,userid = xiaomiyundong.login(user,password)
        if login_token == 0:
            print("登陆失败！")
            return "login fail!"
    
        t = xiaomiyundong.get_time()
        
        app_token = xiaomiyundong.get_app_token(login_token)
    ...
    if len(user_list) == len(passwd_list):
        ...
        for line in range(0,len(user_list)):
            if len(setp_array) == 2:
                ...
                print ("已设置为随机步数（"+setp_array[0]+"-"+setp_array[1]+"）")
            elif str(step) == '0':
                step = ''
            push += xiaomiyundong.main(user_list[line], passwd_list[line], step) + '\n'
    else:
        print('用户名和密码数量不对')
